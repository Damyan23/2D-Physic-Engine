...
namespace GXPEngine {
	...
	public class Gizmos {
        ...
        /// using DrawLine.
        /// </summary>
        public static void DrawRectangle(float xCenter, float yCenter, float width, float height, float rotationDegrees = 0f, GameObject space = null, uint color = 0, byte lineWidth = 0)
        {
            // Convert degrees to radians
            float radians = rotationDegrees * Mathf.PI / 180f;

            // Pre-calculate cos and sin of rotation
            float cos = Mathf.Cos(radians);
            float sin = Mathf.Sin(radians);

            // Define the 4 corners relative to the center (unrotated)
            Vector2[] corners = new Vector2[4];
            corners[0] = new Vector2(-width / 2, -height / 2); // Top-left
            corners[1] = new Vector2(width / 2, -height / 2);  // Top-right
            corners[2] = new Vector2(width / 2, height / 2);   // Bottom-right
            corners[3] = new Vector2(-width / 2, height / 2);  // Bottom-left
            ...
            for (int i = 0; i < corners.Length; i++)
            {
                ...
                corners[i] = new Vector2(rotatedX + xCenter, rotatedY + yCenter);
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                DrawLine(start.x, start.y, end.x, end.y, space, color, lineWidth);
            }
        }
